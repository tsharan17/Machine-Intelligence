    ...
    def generate_loop(self, pin_assignments: dict, action: dict) -> str:
        ...
        if action_type == "blink" and times > 0:
            lines = [
                ...
                f"        delay({interval});",
                f"    }}",
                f"    while(true);  // halt after {times} beeps",
            ]
            return "\n".join(lines) + "\n"
        ...
        elif action_type == "detect_distance":
            # Buzzer is triggered by ultrasonic — its pin is injected into
            # the ultrasonic loop block via _enrich_actions() in firmware_builder.py
            return f"    // BUZZER on GPIO {pin} is triggered by ULTRASONIC logic above\n"
